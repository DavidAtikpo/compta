...
export default function AdminStructuresPage() {
  ...
  const [data, setData] = useState<Paginated<AdminStructure> | null>(null);
  ...
  const setUrl = (nextPage: number) => {
    ...
    router.push(`/admin/structures${params.toString() ? `?${params.toString()}` : ""}`);
  };
  ...
  useEffect(() => {
    const boot = async () => {
      ...
      try {
        ...
        const res = await fetch(`/api/admin/structures?page=${page}&pageSize=${pageSize}`, { headers: authHeaders });
        if (!res.ok) throw new Error();
        setData((await res.json()) as Paginated<AdminStructure>);
      } catch {
  ...
  const deleteStructure = async (id: string) => {
    if (!window.confirm("Supprimer cette structure ? (soft delete)")) return;
    setBusyId(id);
    setError("");
    try {
      ...
      setError("Suppression structure échouée.");
    } finally {
      setBusyId("");
    }
  };
        ...
        {data.items.map((entry) => (
          <div key={entry.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-slate-200 px-3 py-2">
            <div>
              ...
              <p className="text-xs text-slate-500">
                {entry.region} — {entry.type} — {entry.siret || "Sans SIRET"} — {entry.user?.email || "Sans utilisateur"}
              </p>
            </div>
            <button
              disabled={!canDelete || busyId === entry.id}
              onClick={() => void deleteStructure(entry.id)}
