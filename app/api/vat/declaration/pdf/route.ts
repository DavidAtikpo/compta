...
import { buildVatDeclarationPdf } from "@/lib/vat-declaration-pdf";
...
export async function GET(request: NextRequest) {
  ...
  const pdfBytes = await buildVatDeclarationPdf(declaration);

  const filename =
    type === "CA12"
      ? `TVA_CA12_${year}.pdf`
      : `TVA_CA3_${year}-${String(month ?? 1).padStart(2, "0")}.pdf`;
